...
int log_fault(unsigned char fault_type, unsigned char hour, unsigned char minute, unsigned char second);


void uart_init(void);
void uart_tx_char(char c);
void uart_tx_string(const char *s);
void uart_send_fault_log(fault_type_t fault_type, unsigned char hour, unsigned char minute, unsigned char second);
...
fault_context_t last_fault_context = {0, 0};


static unsigned char fault_uart_sent = 0;


static unsigned char rb1_prev = 1;


static unsigned char last_good_hour = 0;
static unsigned char last_good_min = 0;
static unsigned char last_good_sec = 0;


static void busy_delay_ms(unsigned int ms) {
...
void uart_send_fault_log(fault_type_t fault_type, unsigned char hour, unsigned char minute, unsigned char second)
{
    ...
    switch (fault_type) {
        case FAULT_SDA_STUCK:
            fault_name = "SDA STUCK";
            break;
        case FAULT_SCL_STUCK:
            fault_name = "SCL STUCK";
            break;
        case FAULT_NACK:
            fault_name = "NACK";
            break;
    ...
    sprintf(message, "Fault:%s Time:%02u:%02u:%02u\r\n",
            fault_name, hour, minute, second);
    uart_tx_string(message);
}



int wait_i2c_idle(void)
{
    timeout_counter = 6000;
...
int address(char addr)
{
    if (!wait_i2c_idle()) return 0;
    SSPBUF = addr;
    if (!wait_sspif_and_clear()) return 0;
    if (ACKSTAT) return 0;
    return 1;
}
...
void I2C_bus_recovery(void)
{
    ...
    for (i = 0; i < 9; ++i)
    {
        PORTCbits.RC3 = 1; _delay((unsigned long)((50)*(8000000/4000000.0)));
        PORTCbits.RC3 = 0; _delay((unsigned long)((50)*(8000000/4000000.0)));
        if (PORTCbits.RC4 == 1) break;
    }
...
int rtc_read(unsigned char *hour, unsigned char *min, unsigned char *sec)
{
    int d;
    unsigned char data;

    if (!start()) return 0;
    if (!address(0xD0)) { stop(); return 0; }
    if (!write(0x00)) { stop(); return 0; }
    if (!repeated_start()) { stop(); return 0; }
    if (!address(0xD1)) { stop(); return 0; }

    d = read_byte(0);
    if (d < 0) { stop(); return 0; }
...
{
    if (!start()) return 0;
    if (!address(0xA0)) { stop(); return 0; }
    if (!write(addr >> 8)) { stop(); return 0; }
    if (!write(addr & 0xFF)) { stop(); return 0; }
    if (!write(data)) { stop(); return 0; }
    if (!stop()) return 0;
    _delay((unsigned long)((5)*(8000000/4000.0)));
    return 1;
}

int eeprom_read_byte(unsigned int addr, unsigned char *data)
{
    int d;
    if (!start()) return 0;
    if (!address(0xA0)) { stop(); return 0; }
...
void save_log_state(void)
{
    internal_eeprom_write(5, (unsigned char)(write_index & 0xFF));
    internal_eeprom_write(6, (unsigned char)(write_index >> 8));
    internal_eeprom_write(7, (unsigned char)(log_count & 0xFF));
    internal_eeprom_write(8, (unsigned char)(log_count >> 8));
    internal_eeprom_write(9, 0xA5);
}


void restore_log_state(void)
{

    if (internal_eeprom_read(9) != 0xA5)
    {
        write_index = 0;
        log_count = 0;
        save_log_state();

        internal_eeprom_write(4, 0);
        return;
    }


    write_index = ((unsigned int)internal_eeprom_read(6) << 8)
                 | internal_eeprom_read(5);
    log_count = ((unsigned int)internal_eeprom_read(8) << 8)
                 | internal_eeprom_read(7);


    if (write_index >= 256) write_index = 0;
    if (log_count >= 256) log_count = 0;
}



void buffer_fault_in_internal_eeprom(unsigned char fault_type, unsigned char hour, unsigned char minute, unsigned char second)
{
    if (internal_eeprom_read(4) != 0) return;

    __asm("clrwdt");
    internal_eeprom_write(0, fault_type);
    __asm("clrwdt");
    internal_eeprom_write(0 + 1, hour);
    __asm("clrwdt");
    internal_eeprom_write(0 + 2, minute);
    __asm("clrwdt");
    internal_eeprom_write(0 + 3, second);
    __asm("clrwdt");
    _delay((unsigned long)((10)*(8000000/4000.0)));
    __asm("clrwdt");
    internal_eeprom_write(4, 1);
}

int flush_internal_eeprom_fault(void)
{
    ...
    if (internal_eeprom_read(4) != 1) {
        return 0;
    }

    fault_type = internal_eeprom_read(0);
    hour = internal_eeprom_read(0 + 1);
    minute = internal_eeprom_read(0 + 2);
    second = internal_eeprom_read(0 + 3);

    if (log_fault(fault_type, hour, minute, second)) {
        internal_eeprom_write(4, 0);
        fault_uart_sent = 0;
        return 1;
    }
...
int log_fault(unsigned char fault_type, unsigned char hour, unsigned char minute, unsigned char second)
{
    unsigned int addr = write_index * 4;
    if (!eeprom_write_byte(addr, fault_type)) return 0;
    _delay((unsigned long)((5)*(8000000/4000.0)));
    if (!eeprom_write_byte(addr + 1, hour)) return 0;
    _delay((unsigned long)((5)*(8000000/4000.0)));
    if (!eeprom_write_byte(addr + 2, minute)) return 0;
    _delay((unsigned long)((5)*(8000000/4000.0)));
    if (!eeprom_write_byte(addr + 3, second)) return 0;
    _delay((unsigned long)((5)*(8000000/4000.0)));

    write_index++;
    if (write_index >= 256) write_index = 0;
    if (log_count < 256) log_count++;
...
fault_type_t classify_fault(void)
{
    if (last_fault_context.ackstat_set) {
        return FAULT_NACK;
    }
    if (PORTCbits.RC3 == 0) {
        return FAULT_SCL_STUCK;
    }
    if (PORTCbits.RC4 == 0) {
        return FAULT_SDA_STUCK;
    }
    return FAULT_TIMEOUT;
}

int read_temp_with_classification(int *temp_data)
{
    unsigned char msb, lsb;
    int d;

    if (!start()) {
        last_fault_context.ackstat_set = 0;
        last_fault_context.failed_step = 1;
        return 0;
    }
    if (!address(0x90)) {
        last_fault_context.ackstat_set = ACKSTAT;
        last_fault_context.failed_step = 2;
        stop();
        return 0;
    }
    if (!write(0x00)) {
        last_fault_context.ackstat_set = ACKSTAT;
        last_fault_context.failed_step = 3;
        stop();
        return 0;
    }
    if (!repeated_start()) {
        last_fault_context.ackstat_set = 0;
        last_fault_context.failed_step = 4;
        stop();
        return 0;
    }
    if (!address(0x91)) {
        last_fault_context.ackstat_set = ACKSTAT;
        last_fault_context.failed_step = 5;
        stop();
        return 0;
    }
    d = read_byte(0);
    if (d < 0) {
        last_fault_context.ackstat_set = 0;
        last_fault_context.failed_step = 6;
        stop();
        return 0;
    }
    msb = d;
    ...
    if (d < 0) {
        last_fault_context.ackstat_set = 0;
        last_fault_context.failed_step = 7;
        stop();
        return 0;
    }
    lsb = d;
    ...
    if (!stop()) {
        last_fault_context.ackstat_set = 0;
        last_fault_context.failed_step = 8;
        return 0;
    }

    sign = 1 & (msb >> 7);
    msb = msb & (~(1 << 7));
    *temp_data = (msb << 1) | (lsb >> 7);
...
void handle_fault(fault_type_t fault_type, unsigned char hour, unsigned char minute, unsigned char second)
{

    switch (fault_type) {
        case FAULT_SDA_STUCK:
            print(0xC0, "Status:SDA STUCK");
            break;
        case FAULT_SCL_STUCK:
            print(0xC0, "Status:SCL STUCK");
            break;
        case FAULT_NACK:
            print(0xC0, "Status:NACK     ");
            break;
        case FAULT_TIMEOUT:
            print(0xC0, "Status:TIMEOUT  ");
            break;
        default:
            print(0xC0, "Status:UNKNOWN  ");
            break;
    }
    busy_delay_ms(2000);
}
...
void clear_external_eeprom(void)
{
    ...
    lcd_clear();
    print(0x80, "Clearing Log...");
    print(0xC0, "EEPROM CLEARED");
    busy_delay_ms(2000);
    lcd_clear();
}
...
void main(void)
{
    PORTA = 0x00; TRISA = 0xFF;
    PORTC = 0x00; TRISC = 0x98;
    ...
    if ((PCON & 0x10) == 0) {
        unsigned char h, m, s;
        if (rtc_read(&h, &m, &s)) {
            log_fault(FAULT_WATCHDOG, h, m, s);

            uart_send_fault_log(FAULT_WATCHDOG, h, m, s);
        }
    }
    ...
    while (1)
    {
        ...
        unsigned char rb1_curr = PORTBbits.RB1;


        if (rb1_prev == 1 && rb1_curr == 0) {

            busy_delay_ms(20);
            if (PORTBbits.RB1 == 0) {
                clear_external_eeprom();
            }
        }
        ...
        {
            if (read_temp_with_classification(&temperature)) {
                ok = 1;
                break;
            }
        }
        ...
        if (!ok) {
            fault_type_t fault_type = classify_fault();
            unsigned char hour, minute, second;

            if (!rtc_read(&hour, &minute, &second)) {
                hour = last_good_hour;
                minute = last_good_min;
                second = last_good_sec;
            }


            if (!fault_uart_sent) {
                uart_send_fault_log(fault_type, hour, minute, second);
                fault_uart_sent = 1;
            }

            buffer_fault_in_internal_eeprom(fault_type, hour, minute, second);
            I2C_bus_recovery();
            handle_fault(fault_type, hour, minute, second);
            continue;
        }


        flush_internal_eeprom_fault();


        rtc_read(&last_good_hour, &last_good_min, &last_good_sec);

        lcd_clear();

        char temp_str[6];
        int abs_temp = (temperature < 0) ? -temperature : temperature;
        temp_str[0] = (temperature < 0) ? '-' : '+';
        temp_str[1] = (abs_temp / 100) + '0';
        temp_str[2] = ((abs_temp / 10) % 10) + '0';
        temp_str[3] = (abs_temp % 10) + '0';
        temp_str[4] = 'C';
        temp_str[5] = '\0';
        print(0x80, temp_str);

        print(0xC0, "Status:SYSTEM OK");
